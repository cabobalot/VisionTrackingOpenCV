...
const String window_Contours_name = "Contour Window";
...
int main(int argc, char* argv[])
{
    ...
    namedWindow(window_Contours_name);
	...
	// erode + dialate
	Mat kernal = Mat();
	dilate(frame_threshold, frame_contours, kernal, Point(-1, -1), 4);
	erode(frame_contours, frame_contours, kernal, Point(-1, -1), 4);
	...
	for( int i = 0; i < contours.size(); i++ ) {
		...
		drawContours(frame_contours , contours, i, color, 2, 8, hierarchy, 0, Point());
	...
	imshow(window_Contours_name, frame_contours);
        //! [show]
